...
class ViewFactory(private val context: Context, private val cb: Callbacks) {
    ...
    var docListContainer: LinearLayout? = null
    ...
    private fun buildToolbar(dp: Float): FrameLayout {
        ...
        val mainBar = LinearLayout(context).apply {
            ...
            val tCell = buildCategoryCell("工具", toolbarH, dp) { cb.onToolsToggle() }
            toolsCell = tCell
            addView(tCell)
    ...
    fun buildCategoryCell(label: String, toolbarH: Int, dp: Float, onClick: () -> Unit): LinearLayout =
    ...
    fun buildPunctButton(punct: String, dp: Float): TextView =
        TextView(context).apply {
            ...
            val hPad = (10 * dp).roundToInt(); val vPad = (6 * dp).roundToInt()
    ...
    fun buildAllToolsPanel(dp: Float): LinearLayout {
        ...
        val docListLayout = LinearLayout(context).apply {
            orientation = LinearLayout.VERTICAL
            layoutParams = LinearLayout.LayoutParams(MP, WC)
        }
    ...
    fun buildBgColorPanel(dp: Float) = buildColorSwatchRow(
        dp, AppConfig.BG_COLORS,
        extra = ColorOption(context.getColor(R.color.divider), "自訂") to { cb.onImagePickerRequested() }
    ...
    fun buildFontRow(dp: Float): View = LinearLayout(context).apply {
        ...
        sizeChip.setOnClickListener {
            showPopupSeekbar(
                ...
                initial = AppConfig.FONT_SIZE_LIST.indexOfFirst { e -> e.first == cb.getFontSizeSp() }.coerceAtLeast(0),
        ...
        addView(label("字距"))
        val gapChip = chip(cb.getWordGapDp().toInt().toString())
        ...
        gapChip.setOnClickListener {
            showPopupSeekbar(
                ...
                max     = 20,
                min     = 3,
                initial = cb.getWordGapDp().toInt().coerceAtLeast(3),
                format  = { i -> "$i" }
            ) { p ->
                gapValueLabelRef?.text = "$p"
                cb.onWordGapChanged(p)
            }
        }
        addView(gapChip)
    }
            ...
            listOf(InputMode.SCATTER to "灑花輸入", InputMode.SEQUENTIAL to "連續輸入")
                .forEach { (mode, label) ->
                    addView(TextView(context).apply {
                        ...
                        setTextColor(context.getColor(R.color.text_dark))
                        background = GradientDrawable().apply {
                            setColor(if (cb.getInputMode() == mode) context.getColor(R.color.chip_active)
    ...
    // ── Doc action row ────────────────────────────────────────────────────
    fun buildDocActionRow(dp: Float): LinearLayout =
        LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            layoutParams = LinearLayout.LayoutParams(MP, WC)
            ...
            setPadding(hPad, vPad, hPad, vPad)
            listOf(
                "更名" to Runnable { cb.onRenameSession() },
                "新增" to Runnable { cb.onNewSession() },
                "全部" to Runnable { cb.onShowAllSessions() }
    ...
    fun buildSelectionOptionsView(dp: Float): LinearLayout {
        ...
        fun sep() = View(context).apply {
            layoutParams = LinearLayout.LayoutParams((90 * dp).roundToInt(), (1f * dp).roundToInt()).also {
    ...
    fun buildHandlePasteView(dp: Float): TextView =
        TextView(context).apply {
            ...
            gravity = Gravity.CENTER
            setTextColor(context.getColor(R.color.text_dark))
            val hP = (14 * dp).roundToInt(); val vP = (8 * dp).roundToInt()
            setPadding(hP, vP, hP, vP)
    ...
    fun buildSelectionHandle(isStart: Boolean, dp: Float): View {
            ...
            addView(SeekBar(context).apply {
                ...
                setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                    override fun onProgressChanged(sb: SeekBar?, p: Int, fromUser: Boolean) {
                        if (fromUser) { val v = p + min; valueLabel.text = format(v); onChange(v) }
                    }
